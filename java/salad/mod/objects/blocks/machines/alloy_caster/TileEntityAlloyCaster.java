...
public class TileEntityAlloyCaster extends TileEntity implements ITickable
{
	...
	{
		return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("container.alloy_caster");
	}
	...
	public static boolean isBurning(TileEntityAlloyCaster te) 
	{
	...
	{	
		...
		{
			--this.burnTime;
			BlockAlloyCaster.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = AlloyCasterRecipes.getInstance().getSinteringResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = AlloyCasterRecipes.getInstance().getSinteringResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
